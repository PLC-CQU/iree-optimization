...
本文总结当前阶段针对 IREE 动态 shape 模型的优化工作。重点是直接优化动态 shape 编译路径，希望让单个动态模型接近静态模型的性能。
...
## 11. bounded dynamic 的定位

bounded dynamic 当前表示：
...
```text
对于普通 matmul，不需要 bounded range，safe dynamic-M 已经有效。
...
```bash
cd /home/zhongjialin/projects/iree-optimization/scripts/deepseek

GPU=2 B=1 S=32 REPETITIONS=3 MIN_TIME=1x WARMUP_TIME=0 BENCH_TIMEOUT=60s \
  ./benchmark_bounded_dynamic_shape.sh
```
...
GPU=2 REPETITIONS=3 MIN_TIME=1x WARMUP_TIME=0 BENCH_TIMEOUT=120s \
  ./benchmark_dynamic_simt_context_grid.sh
```
...
```text
scripts/deepseek/benchmark_bounded_dynamic_shape.sh
```
...
```text
scripts/deepseek/benchmark_dynamic_simt_context_grid.sh
```
...
```bash
GPU=2 B=1 S=64  CANDIDATE_DYNAMIC_VMFB=/tmp/deepseek_dynamic_bs_bounded_b8_s128_mma_flat_context.vmfb OUT_DIR=/tmp/iree_mma_flat_context_correctness_b1_s64  scripts/deepseek/check_safe_dynamic_m_correctness.sh
GPU=2 B=4 S=64  CANDIDATE_DYNAMIC_VMFB=/tmp/deepseek_dynamic_bs_bounded_b8_s128_mma_flat_context.vmfb OUT_DIR=/tmp/iree_mma_flat_context_correctness_b4_s64  scripts/deepseek/check_safe_dynamic_m_correctness.sh
GPU=2 B=8 S=128 CANDIDATE_DYNAMIC_VMFB=/tmp/deepseek_dynamic_bs_bounded_b8_s128_mma_flat_context.vmfb OUT_DIR=/tmp/iree_mma_flat_context_correctness_b8_s128 scripts/deepseek/check_safe_dynamic_m_correctness.sh
```

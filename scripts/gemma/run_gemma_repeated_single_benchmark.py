#!/usr/bin/env python3
"""Run repeated single-shot Gemma benchmarks and aggregate baseline vs flatten."""
...
def paths(exp_name: str, variant: str):
    ...
    if variant == "baseline":
        return {
            "module": exp / "standard_cuda" / "gemma_cuda.vmfb",
            "params": exp / "standard_cuda" / "gemma_params.irpa",
            "log_dir": exp / "logs" / "repeated_single_baseline",
        }
    return {
        "module": exp / "standard_cuda_flatten_matmul" / "gemma_flatten_cuda.vmfb",
        "params": exp / "standard_cuda_flatten_matmul" / "gemma_flatten_params.irpa",
...
def main():
    ...
    if baseline and flat and baseline.get("successful_runs") and flat.get("successful_runs"):
        ...
        performance["latency_reduction_percent"] = (1.0 - flat_ms / base_ms) * 100.0 if base_ms else None
    ...
    summary = {
        "metadata": {
            ...
            "gpu": args.gpu,
            "cuda_async_allocations": args.cuda_async_allocations,

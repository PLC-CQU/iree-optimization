module {
  func.func @main(%lhs: tensor<?x?x256xf32>,
      ...
      -> (tensor<?x?x256xf32>, tensor<?x?x256xf32>, tensor<?x?x256xf32>)
    ...
    %k = linalg.generic {
        ...
        iterator_types = ["parallel", "parallel", "parallel", "reduction"]
      } ins(%lhs, %wk : tensor<?x?x256xf32>, tensor<256x256xf32>)
        outs(%init : tensor<?x?x256xf32>) {
      ^bb0(%l: f32, %r: f32, %acc: f32):
        %mul = arith.mulf %l, %r : f32
        %add = arith.addf %mul, %acc : f32
    ...
    %v = linalg.generic {
        ...
        iterator_types = ["parallel", "parallel", "parallel", "reduction"]
      } ins(%lhs, %wv : tensor<?x?x256xf32>, tensor<256x256xf32>)
        outs(%init : tensor<?x?x256xf32>) {
      ^bb0(%l: f32, %r: f32, %acc: f32):
        %mul = arith.mulf %l, %r : f32
        %add = arith.addf %mul, %acc : f32
    ...
    return %q, %k, %v : tensor<?x?x256xf32>, tensor<?x?x256xf32>, tensor<?x?x256xf32>
  }
}
